...
public class UIPanel : UIRect
{
	...
	/// </summary>
	private Vector2 screensize = Vector2.one;
	public Vector2 GetWindowSize ()
	{
		UIRoot rt = root;
		//Vector2 size = NGUITools.screenSize; //sn
		screensize.x = Screen.width;
		screensize.y = Screen.height;
		//if (rt != null) size *= rt.GetPixelSizeAdjustment(Mathf.RoundToInt(size.y)); //sn
		if (rt != null) screensize *= rt.GetPixelSizeAdjustment(Mathf.RoundToInt(screensize.y));
		return screensize;
	}
