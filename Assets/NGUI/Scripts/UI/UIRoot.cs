...
public class UIRoot : MonoBehaviour
{
	...
	/// </summary>
	private Vector2 screen = Vector2.one;
	...
	public float pixelSizeAdjustment
	{
		get
		{
			//int height = Mathf.RoundToInt(NGUITools.screenSize.y); //sn
	...
	private Vector3 use_localscaleuse =Vector3.one;
	private Vector3 ls =Vector3.one;
	private float size =0f;
	private float calcActiveHeight = 0f;
	public void UpdateScale (bool updateAnchors = true)
	{
		...
		if (mTrans != null)
		{
			 ...
			 calcActiveHeight = activeHeight;

			if (calcActiveHeight > 0f)
			{
				size = 2f / calcActiveHeight;

				 ls = mTrans.localScale;

				if (!(Mathf.Abs(ls.x - size) <= float.Epsilon) ||
					!(Mathf.Abs(ls.y - size) <= float.Epsilon) ||
					!(Mathf.Abs(ls.z - size) <= float.Epsilon))
				{
					use_localscaleuse.x = size;
					use_localscaleuse.y = size;
					use_localscaleuse.z = size;
					mTrans.localScale = use_localscaleuse;//new Vector3(size, size, size);
