...
public class AnimMapBakerWindow : EditorWindow {
    ...
    private static void SaveAsPrefab(ref BakedData data)
    {
        ...
        var folderPath = CreateFolder();                                                            ////
        string meshName = data.Name.Split("_")[0];
        string filePath = Path.Combine(folderPath, $"{meshName}.mesh");                            ////
        Mesh _mesh = null;
        if (!File.Exists(filePath))
        {
            AssetDatabase.CreateAsset(data.MeshData, filePath);
        }
        _mesh = AssetDatabase.LoadAssetAtPath<Mesh>(filePath);    ////

        go.AddComponent<MeshFilter>().sharedMesh = _mesh;//// data.MeshData;////_targetGo.GetComponentInChildren<SkinnedMeshRenderer>().sharedMesh;////
